...
def harmonic_non_max_suppression(point_coords, point_probs, shape_descriptors, overlap_thresh=0.5, dim_scale=(1,1,1), num_kernel=1, point_limit=3000, **kwargs):

    if len(point_coords)>point_limit:
        
        print_timestamp('Too many points, aborting NMS')
        nms_coords = point_coords[:point_limit]
        nms_probs = point_probs[:point_limit]
        nms_shapes = shape_descriptors[:point_limit]
